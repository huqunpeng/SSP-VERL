...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='facebook_combined', help="Dataset")
    ...
    folder_path = f'./results/algorithms_{args.dataset}_{args.ratio}_{args.train_type}'
    ...
    # 获取最终最佳算法
    final_best_algorithm = algorithm_pool.get_best_algorithm()

    # 获取评分与节点
    final_score, final_seed_nodes = scoring_module.score_nodes_with_result(final_best_algorithm, timeout=60)

    # 保留前 50 个节点并格式化成字符串
    final_seed_nodes = final_seed_nodes[:50]
    formatted_nodes = '【' + ', '.join(map(str, final_seed_nodes)) + '】'

    # 保存路径
    seed_save_path = os.path.join(folder_path, 'final_top50_seeds.txt')
    with open(seed_save_path, 'w') as f:
        f.write(formatted_nodes)

    print(f"Final Top-50 seed nodes saved to: {seed_save_path}")
