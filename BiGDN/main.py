...
def main(args):
    ...
    test_graph = nx.read_edgelist('test_graphs/facebook_combined.txt', nodetype=int, create_using=nx.DiGraph)
    for u, v, a in test_graph.edges(data=True):
        a['weight'] = 1 / test_graph.in_degree(v)
    test_graphs.append(test_graph)

    env = GraphEnvironment(train_graphs, k, gamma, n_steps, R=100, num_workers=8)
    test_env = GraphEnvironment(test_graphs, k, gamma, n_steps, R=100, num_workers=8)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", type=int, default=20, help="The number of selected seed nodes.")
    ...
    parser.add_argument("--buffer_size", type=int, default=20000, help="The size of the experience replay pool.")
    parser.add_argument("--num_features", type=int, default=64, help="Feature dimension.")
    parser.add_argument("--num_epochs", type=int, default=100, help="The number of training rounds.")
    parser.add_argument("--R", type=int, default=100, help="The frequency of dissemination.")
